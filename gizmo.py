...
class Gizmo(Model):
    ...
    def render_collision_check(self, scale, is3d=True, rotation=True):
        if not self.hidden:
            glPushMatrix()
            ...
            if is3d and rotation: named_meshes["rotation_x"].render_colorid(0x4)
            if rotation: named_meshes["rotation_y"].render_colorid(0x5)
            if is3d and rotation: named_meshes["rotation_z"].render_colorid(0x6)
    ...
    @catch_exception
    def render(self, is3d=True, rotation=True):
        if not self.hidden:
            glColor4f(*X_COLOR)
            ...
            if is3d and rotation: self.named_meshes["rotation_z"].render()
            glColor4f(*MIDDLE)
    ...
    def render_scaled(self, scale, is3d=True, rotation=True):
        glPushMatrix()
        glTranslatef(self.position.x, -self.position.z, self.position.y)
